...
from pyfisheye.utils.common import build_companion_matrix
...
def backproject_fast(points: np.ndarray,
                     lookup_theta: np.ndarray,
                     lookup_rho: np.ndarray,
                     distortion_centre: np.ndarray,
                     scaling_matrix: Optional[np.ndarray] = None) -> np.ndarray:
    """
    Use a lookup table to perform a faster backprojection of 3D points onto the image space.
    :param points: The points to backproject, can be any shape as long as the last dimension
        has length 2.
    ...
    :returns: The backprojected pixels.
    """
    if scaling_matrix is None:
        scaling_matrix = np.eye(2, dtype=np.float64)
    if not np.issubdtype(points.dtype, np.floating):
